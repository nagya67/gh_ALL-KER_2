...
const make_hangsorismetlesek_table = function () {
    ...
    let myHangSorIsmetlodes_Results = []
    let myhangSorIsmetlodes_Indexes = []
    myHangSorIsmetlodes_Results.push(hangSorIsmetlodes_Results[0])
    for (let i = 1; i < hangSorIsmetlodes_Results.length; i++) {
        if (hangSorIsmetlodes_Results[i - 1][0] != hangSorIsmetlodes_Results[i][0]) {
            myHangSorIsmetlodes_Results.push(hangSorIsmetlodes_Results[i])
            hangSorIsmetlodes_Indexes.push(hangSorIsmetlodes_Indexes[i])
        }
    }
    hangSorIsmetlodes_Results = myHangSorIsmetlodes_Results.slice(0)
    hangSorIsmetlodes_Indexes = myhangSorIsmetlodes_Indexes.slice(0)
...
const HI_Filter_Function = function () {
    make_hangsorismetlesek_table();
    HI_info_function();
    HI_distributions_function();
    graphOfDistributionOfHI()
...
HI_parts_how_many = 0
let myHangSorIsmetlodes_Results = []
let hangSorIsmetlodesekShortText_Results = []

const HI_short_texts_function2 = function () {
    HI_radius_function()
    HI_parts_how_many = 0
    myHangSorIsmetlodes_Results = []
    hangSorIsmetlodesekShortText_Results = []
    let myArray = [];
    for (let i = 0; i < hangSorIsmetlodes_Results.length; i++) {
        if (hangSorIsmetlodes_Results[i][3] == 1) {
            myArray.push(hangSorIsmetlodes_Results[i])
        }
    }

    myHangSorIsmetlodes_Results = myArray.slice(0)

    // myHangSorIsmetlodes_Results.push(hangSorIsmetlodes_Results[0])
    // for (let i = 1; i < hangSorIsmetlodes_Results.length; i++) {
    //     if (hangSorIsmetlodes_Results[i][3] == 1 &&
    //         hangSorIsmetlodes_Results[i - 1][0] != hangSorIsmetlodes_Results[i][0]) {
    //         myHangSorIsmetlodes_Results.push(hangSorIsmetlodes_Results[i])
    //     }
    // }

    for (let i = 0; i < myHangSorIsmetlodes_Results.length; i++) {
        let myArray2 = [myHangSorIsmetlodes_Results[i][0] - HI_radius, 'finish', [], [], []]
        HI_stop = false
        //myArray2: [start, finish, [kifejezés1 helye, kifejezés2 helye,...],[kifejezés1, kifejezés2,...],
        //           [ismételt_hangsor1, ismételt_hangsor2]]
        while (HI_stop == false) {
            myArray2[2].push(myHangSorIsmetlodes_Results[i][0])
            myArray2[3].push(myHangSorIsmetlodes_Results[i][2])
            myArray2[4].push(myHangSorIsmetlodes_Results[i][1])
            if (i == myHangSorIsmetlodes_Results.length - 1 ||
                myHangSorIsmetlodes_Results[i + 1][0] - myHangSorIsmetlodes_Results[i][0] > HI_radius) {
                myArray2[1] = myHangSorIsmetlodes_Results[i][0] + HI_radius;
                HI_stop = true
            }
            i += 1
        }
        hangSorIsmetlodesekShortText_Results.push(myArray2)
        i -= 1
    }
    HI_parts_how_many = hangSorIsmetlodesekShortText_Results.length
}


let hangSorIsmetlodes_Final_Results2 = ''
const HI_short_texts_coloring_function = function () {
    hangSorIsmetlodes_Final_Results2 = ""
    let myText1 = ""
    let myText2 = ""
    let myStart = 0
    let myFinish = 0
    ...
    let nextPartOfText = ""
    let counter = 1;
    for (let i = 0; i < hangSorIsmetlodesekShortText_Results.length; i++) {
        myStart = hangSorIsmetlodesekShortText_Results[i][0]
        myFinish = hangSorIsmetlodesekShortText_Results[i][1]
        keyWords = hangSorIsmetlodesekShortText_Results[i][3]
        hangsor = hangSorIsmetlodesekShortText_Results[i][4]
        myText1 = "";
        myStart = Math.max(0, myStart)
        myFinish = Math.min(myFinish, allWordsORIGINAL.length)
        for (let k = myStart; k < myFinish; k++) {
            myText1 += allWords[k] + ' '
        }
        myText2 = ''
        mySplitArray = []
        ...
        for (let j = 0; j < keyWords.length; j++) {
            if (j == 0) {
                mySplitArray = myText1.split(keyWords[j])
                nextPartOfText = mySplitArray[1]
            } else {
                mySplitArray = nextPartOfText.split(keyWords[j])
            }
            myText2 += mySplitArray[0]
            myText2 += `<span class='dark-olive'>${keyWords[j]}</span>`
            myText2 += ` <span class='dark-pink'>(${hangsor[j]}) </span>`
            nextPartOfText = mySplitArray[1]
            if (j == keyWords.length - 1) {
                myText2 += mySplitArray[1]
            }
        }
        counter += 1
        myText2 += `<br><hr>`
        hangSorIsmetlodes_Final_Results2 += myText2
    }
    document.querySelector('#hangsorismétlések-short-texts').innerHTML = hangSorIsmetlodes_Final_Results2
}


const HI_function_2 = function () {
    HI_short_texts_function2();
    HI_short_texts_coloring_function();
    HI_info_function();
    HI_info_function2()
    HI_distributions_function();
    graphOfDistributionOfHI()
...
const HI_info_function2 = function () {
    HI_data3 = HI_parts_how_many
    document.querySelector('#HI-info-1_').innerHTML = `<b>${HI_data1}</b>`
    document.querySelector('#HI-info-2_').innerHTML = `<b>${HI_data2}</b>`
    document.querySelector('#HI-info-3_').innerHTML = `<b>${HI_data3}</b>`
}
